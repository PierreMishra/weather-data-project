...
* The SQLite3 database is stored in `database` folder. Due to the GitHub size limits, the `weather.db` file only contains an empty data model. You would have to run the code locally to populate the records.
...
Based on your OS and python version installed, use one of the following to activate the virtual environment
...
```
NOTE: Alternatively in Windows, `venv\bin\activate` may be used depending on your python installation.

* #### Installing python package requirements in the virtual environment
Before installing pacakges from `requirements.txt`, run the following:
```sh
pip3 install reverse_geocode==1.4.1
```
NOTE: Installing this pacakge from `requirements.txt` throws dependency errors. That is why we need to install it in a separate step. This package is a prerequisite to another reverse geocoding library in the `requirements.txt` file which is important for our database purposes. The package helps to retrieve the US state for each weather station using its latitude and longitude coordinates.
...
We can use the `Try it out` feature of the Swagger documentation to test our API endpoints. We can filter our data by multiple optional query parameters, as can be seen in this sample screenshot below.
...
`api.py` is associated to the following modules as well:
* `data_model.py`
* `unit_test.py`
...
1. `dim_weather_station` -  Dimension table containing a list of weather stations for which we have data. It stores attributes about the weather stations such as their IDs, station names, and the states in which they are located. It also contains a surrogate key which is used to connect to the fact table `fact_weather_data`.
...
3. `fact_weather_data` - Fact table containing all the raw weather records for each station and date. `station_key` from `dim_weather_station` table and `data_id` from `dim_date` constitutes to the primary composite key for this fact table.
...
5. `reference_nasa_table` - Reference table containing various attributes for weather stations across the US. The full list of stations with latitude and longitude coordinates was found from here `https://data.giss.nasa.gov/gistemp/station_data_v4/station_list.txt` and was parsed and filtered to only contain the stations within the US.
